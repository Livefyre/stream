...
ReadableArray, WritableArray, inherits) {
    describe('stream/insert-ben', function () {
        ...
        describe('bens tests', function () {
            ...
            function RepeatedReadable (repeat, opts) {
                this._repeat = repeat;
                Readable.call(this, opts);
            }
            inherits(RepeatedReadable, Readable);

            RepeatedReadable.prototype._read = function () {
                setTimeout(function () {
                    this.push(this._repeat);
                }.bind(this), 50);
            }
            beforeEach(function () {
                ones = new RepeatedReadable(1);
            });
            afterEach(function () {
                ones.pause();
            });
            it('every(1) works with pipe', function () {
                var twos = new ReadableArray([2,2,2,2,2]);
                var inserter = Insert(twos).every(1);
                var writable = new WritableArray();

                ones.pipe(inserter, { end: false }).pipe(writable);

                waitsFor(function () {
                    return writable.get().length === 10;
                });
                runs(function () {
                    console.log(writable.get());
                    expect(writable.get()).toEqual([1,2,1,2,1,2,1,2,1,2]);
                });
            });
            it('every(2) works with pipe', function () {
                var twos = new ReadableArray([2,2,2,2,2]);
                var inserter = Insert(twos).every(2);
                var writable = new WritableArray();
                ...
                runs(function () {
                    expect(writable.get()).toEqual([1,1,2,1,1,2,1,1,2,1]);
                });
            });
            it('every(3) works with pipe', function () {
                var twos = new ReadableArray([2,2,2,2,2]);
                var inserter = Insert(twos).every(3);
                var writable = new WritableArray();
                ...
                runs(function () {
                    expect(writable.get()).toEqual([1,1,1,2,1,1,1,2,1,1]);
                });
            });
            it('every(4) works with pipe', function () {
                var twos = new ReadableArray([2,2,2,2,2]);
                var inserter = Insert(twos).every(4);
                var writable = new WritableArray();
                ...
                runs(function () {
                    expect(writable.get()).toEqual([1,1,1,1,2,1,1,1,1,2]);
                });
            });
        });
